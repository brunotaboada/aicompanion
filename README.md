...
Use `--dry-run-tokens` to print per-task prompt estimates without spawning an agent or running tests — useful for tuning `task_preamble_strip`, `init_instructions`, and budget sizing:
...
| `--dry-run-tokens` exits 1 with I/O error | Cannot read task files or acquire state lock | Check permissions; ensure no other run holds `state.lock` |
