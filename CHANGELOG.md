...
### Added
- **Fail-fast verify auto-detect** — when `verify_commands` is unset, detect a cheap-first pipeline from `package.json` scripts or Makefile targets (`lint` → `typecheck`/`type-check` → `test`). Explicit `verify_commands` are never reordered. Run startup prints `Verify: a → b → c` so the pipeline is visible
